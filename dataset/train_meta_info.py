...
def get_gso_split(resolution=128):
    ...
    if os.path.exists(gso_split_pkl):
        ...
        if os.path.exists('../data/google_scanned_objects'):
            sym_fns = np.loadtxt('assets/gso_sym.txt',dtype=np.str_).tolist()
            gso_fns = []
...
co3d_categories = np.loadtxt('assets/co3d_names.txt',dtype=np.str_).tolist()
...
shapenet_excluded_instance=np.loadtxt('assets/shapenet_sym_objects.txt', dtype=np.str_).tolist()
