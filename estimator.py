...
class Gen6DEstimator:
    ...
    def _load_module(cfg):
        ...
        logger.debug(f'load from detector_pretrain/model_best.pth step {state_dict["step"]}')
        # gpu_tracker.track()
        refiner.cuda().eval()
        # gpu_tracker.track()
        return refiner
    ...
    def _load_selector_dino_module(cfg):
        ...
        name = selector_cfg["name"]
        state_dict = torch.load(f'../data/checkpoints/selector_pretrain/model_best.pth')
        selector.load_state_dict(state_dict['network_state_dict'], strict = True  )
        logger.debug(f'load from ../data/checkpoints/selector_pretrain/model_best.pth step {state_dict["step"]}')
        selector.cuda().eval()
        return selector
    ...
    def _load_refiner_dino_module(cfg):
        ...
        refiner.load_state_dict(state_dict['network_state_dict'], strict = False  )
        logger.debug(f'load from ../data/checkpoints/{refiner_cfg["name"]}/model_best.pth step {state_dict["step"]}')
        # gpu_tracker.track()
        refiner.cuda().eval()
        # gpu_tracker.track()
