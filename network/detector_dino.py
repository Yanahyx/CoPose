...
class Detector(BaseDetector):
    default_cfg={
        ...
        "train_feats": False,
        "use_dino": True,  # 是否使用 DINOv3 替代 VGG
    }
    def __init__(self, cfg):
        ...
        if self.use_dino:
            # 使用 DINOv3
            self.dino_processor = AutoImageProcessor.from_pretrained("../dinov3-vitl16-pretrain-lvd1689m")
            ...
            # DINOv3 特征投影层：将 1024 维投影到 512 维以匹配 VGG
            self.dino_proj_0 = nn.Sequential(
                nn.Conv2d(self.dino_hidden_size, 512, kernel_size=1, stride=1, padding=0),
                nn.InstanceNorm2d(512, affine=False),
                nn.ReLU(True)
            )
            self.dino_proj_1 = nn.Sequential(
                nn.Conv2d(self.dino_hidden_size, 512, kernel_size=1, stride=1, padding=0),
                nn.InstanceNorm2d(512, affine=False),
                nn.ReLU(True)
            )
            self.dino_proj_2 = nn.Sequential(
                nn.Conv2d(self.dino_hidden_size, 512, kernel_size=1, stride=1, padding=0),
                nn.InstanceNorm2d(512, affine=False),
                nn.ReLU(True)
            )
    ...
    def extract_feats(self, imgs):
        if self.use_dino:
            ...
            num_patches_height = img_height // self.dino_patch_size
            num_patches_width = img_width // self.dino_patch_size
            
            # 获取模型输出
            with torch.inference_mode():
                outputs = self.dino_model(**inputs)
            
            last_hidden_states = outputs.last_hidden_state  # [B, 1+4+num_patches, 1024]
            # 提取 patch features（跳过 cls token 和 register tokens）
            patch_features_flat = last_hidden_states[:, 1 + self.dino_num_register_tokens:, :]  # [B, num_patches, 1024]
            
            # 转换为空间布局
            patch_features = patch_features_flat.permute(0, 2, 1).reshape(
                -1, self.dino_hidden_size, num_patches_height, num_patches_width
            )  # [B, 1024, H_patch, W_patch]
            
            # 获取原始图像尺寸用于下采样
            _, _, h_orig, w_orig = imgs.shape
            
            # 生成三个不同分辨率的特征（对应 VGG 的 x0, x1, x2）
            # x0: 1/8 分辨率
            h0, w0 = h_orig // 8, w_orig // 8
            dino_feat_0 = F.interpolate(
                patch_features, size=(h0, w0), mode='bilinear', align_corners=False
            )  # [B, 1024, h0, w0]
            x0 = self.dino_proj_0(dino_feat_0)  # [B, 512, h0, w0]
            
            # x1: 1/16 分辨率
            h1, w1 = h_orig // 16, w_orig // 16
            dino_feat_1 = F.interpolate(
                patch_features, size=(h1, w1), mode='bilinear', align_corners=False
            )  # [B, 1024, h1, w1]
            x1 = self.dino_proj_1(dino_feat_1)  # [B, 512, h1, w1]
            
            # x2: 1/32 分辨率
            h2, w2 = h_orig // 32, w_orig // 32
            dino_feat_2 = F.interpolate(
                patch_features, size=(h2, w2), mode='bilinear', align_corners=False
            )  # [B, 1024, h2, w2]
            x2 = self.dino_proj_2(dino_feat_2)  # [B, 512, h2, w2]
    ...
    def detect_que_imgs(self, que_imgs):
        ...
        # position: 物体在查询图像中的中心位置 [qn, 2]
        # scale_r2q: 参考到查询的尺度比例 [qn]
